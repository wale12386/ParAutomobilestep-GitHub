  ...
  <body>
    <main class="main" id="top">
      <div class="container-fluid px-0">
        ...
        @include('include.admin.nav',['searchRoute'=>route('modele.search')]);
        ...
        <div class="content">
          <h1>Liste Modele</h1>
          <hr>
          <a href="/admin/modele/add" class="mt-6">
            <button type="button" class="btn btn-primary">Ajouter modele</button>
          </a>
...
<div class="alert alert-soft-success d-flex align-items-center" role="alert">
    ...
    <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
          ...
          <table class="table table-striped mt-6">
            ...
            <tbody>
              @foreach($modeles as $index=>$c)
              <tr>
                <th scope="row">{{$index+1}}</th>
                <td>{{$c->libellemodele}}</td>
                <td>{{$c->marques->libellemarque}}</td>
                <td><a href="" data-bs-toggle="modal" data-bs-target="#modifiermodele{{$c->idmodele}}">
...
@foreach ( $modeles as $index=>$c )
<div class="modal fade" id="modifiermodele{{$c->idmodele}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Modifier modele</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><svg class="svg-inline--fa fa-times fa-w-11 fs--1" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 352 512" data-fa-i2svg=""><path fill="currentColor" d="M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z"></path></svg><!-- <span class="fas fa-times fs--1"></span> Font Awesome fontawesome.com --></button>
      </div>
     
      <div class="modal-body">
        <form class="row g-3" method="POST" action="/admin/modele/update" >
                    

          @csrf

       <fieldset >
          <legend>Caracteristique modele</legend>
        <input type="hidden" name="id" value="{{$c->idmodele}}">
              <div class="col-md-6 mt-4">
                  <label class="form-label" >Libelle modele</label>
                  <input class="form-control" name="libellemodele" type="text" value="{{$c->libellemodele}}">
                  @error('libellemodele')
              ...
              <div class="col-md-6 mt-4">
                  ...
                  <select class="form-control" name="marque" >
                    <option value="{{$c->libellemarque}}">{{$c->marques->libellemarque}}</option>
                    @foreach ($marques as $cc )
                    @if($cc->libellemarque != $c->marques->libellemarque)
                          <option value="{{$cc->libellemarque}}">{{$cc->libellemarque}}</option>
                          @endif
                      @endforeach
                  </select>
                  @error('marque')
