    ...
    <main class="main" id="top">
      <div class="container-fluid px-0">
        ...
        @foreach ($affectation as $a)
          <?php $conducteur = DB::table('conducteurs')->where('CINC', $a->CINC)->first(); ?>
          <h3>cette voiture de matricule {{$a->Matricule}} est affectée au conducteur {{$conducteur->nom}} {{$conducteur->prenom}} à la date {{$a->date_affectation}}.<br> Tu peut le modiffier s'il tu veux</h3>
          <div class="dropdown">
          <button class="btn btn-secondary dropdown-toggle btn-lg" style="width: 500px; height: 50px;" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                Modiffier cette affectation
          </button>
          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="width: 500px; height: 50px;">
          <form action="/admin/affectation/update" method="POST">
            @csrf
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label">Matricule de voiture</label>
              <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="matricule" name="matricule" value="{{$a->Matricule}}">
              @error('matricule')
        ...
        @foreach ($affectation as $a)
          <a href="/admin/affectation/{{$a->CINC}}/archive" ><button type="button" class="btn btn-danger">Annuler l'affectation</button></a>
        @endforeach
        <a href="/admin/voiture" > <button  type="button" class="btn btn-light" >Annuler</button></a>
      </div>
